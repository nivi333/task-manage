...
import { ActivityItem, BurndownPoint, Project, ProjectDashboardData, ProjectMetrics, TaskSummaryItem, TeamMember } from '../types/project';
...
const ProjectDashboardPage: React.FC = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (safeBurndown.length !== (burn as any[])?.length) {
          notificationService.info('Some analytics points were ignored due to invalid data');
        }

        setProject(dashboard.project);
        setMetrics(dashboard.metrics);
        setTaskSummary(Array.isArray(dashboard.taskSummary) ? dashboard.taskSummary : []);
        setTeam(Array.isArray(dashboard.team) ? dashboard.team : []);
        setTimeline(Array.isArray(dashboard.timeline) ? dashboard.timeline! : []);
  ...
  const content = useMemo(() => {
    ...
    if (errorStatus) {
      if (errorStatus === 403) {
        return <Result status="403" title="Unauthorized" subTitle="You do not have access to this project." />;
      }
      if (errorStatus === 404) {
        return <Result status="404" title="Project Not Found" subTitle="The requested project does not exist." />;
      }
      if (errorStatus === 400) {
        return <Result status="error" title="Invalid Project Id" subTitle="Please check the URL and try again." />;
      }
      return <Result status="error" title="Error" subTitle="Failed to load project dashboard." />;
    }

    if (!project || !metrics) {
      return <Alert type="warning" message="Incomplete data" description="Dashboard data is missing. Please try again later." showIcon />;
    }
  ...
  }, [loading, errorStatus, project, metrics, taskSummary, team, timeline, burndown, activity]);

  return (
    <AppLayout title={<HeaderTitle level={3}>{project?.name || 'Project Dashboard'}</HeaderTitle>} contentPadding={16}>
      {content}
    </AppLayout>
  );
};
