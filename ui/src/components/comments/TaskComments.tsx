...
const TaskComments: React.FC<TaskCommentsProps> = ({ taskId }) => {
  ...
  useEffect(() => {
    ...
    const handleUpdate = (c: any) => {
      setComments((prev) =>
        prev.map((p) => (String(p.id) === String(c.id) ? (c as Comment) : p))
      );
    };
  ...
  useEffect(() => {
    const unsubscribe = stompService.subscribeComments(taskId, (evt: any) => {
      ...
      if (evt.type === "new" && evt.comment) {
        setComments((prev) => {
          if (prev.some((p) => String(p.id) === String(evt.comment.id)))
            return prev;
        ...
        try {
          ...
          if (
            uname &&
            typeof evt.comment?.content === "string" &&
            evt.comment.content.includes(`@${uname}`)
          ) {
            notificationService.success("You were mentioned in a comment");
          }
        } catch {}
        ...
        setComments((prev) =>
          prev.map((p) =>
            String(p.id) === String(evt.comment.id)
        ...
        setComments((prev) =>
          prev.filter((p) => String(p.id) !== String(evt.id))
        );
      }
    });
    return () => {
  ...
  const create = useCallback(
    async (content: string, parentCommentId?: string | null) => {
      ...
      try {
        const real = await commentService.create(taskId as any, {
          content,
  ...
  const edit = useCallback(
    async (commentId: string, content: string) => {
      ...
      setComments((prev) =>
        prev.map((c) =>
          String(c.id) === String(commentId) ? { ...c, content } : c
        )
      );
      try {
  ...
  // a11y live announcement removed per request
